...
router.post('/profile', uploadGallery.fields([{ name: 'profileImage', maxCount: 1 }, { name: 'heroImage', maxCount: 1 }]), csrfProtection, audit('celebrity:profile_update', 'Celebrity'), ctrl.updateProfile);

// Events
router.get('/events', ctrl.getEvents);
router.get('/events/create', ctrl.getCreateEvent);
router.post('/events/create', uploadEventBanner.single('banner'), csrfProtection, audit('event:create', 'Event'), ctrl.postCreateEvent);
router.get('/events/:id/edit', ctrl.getEditEvent);
router.post('/events/:id/edit', uploadEventBanner.single('banner'), csrfProtection, audit('event:update', 'Event'), ctrl.postUpdateEvent);
...
router.post('/fan-club/post', uploadGallery.array('media', 10), csrfProtection, audit('fanclub:post', 'Post'), ctrl.postFanClubPost);
